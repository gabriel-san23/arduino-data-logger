...
// --- Mapeamento da EEPROM e Log ---
#define ADDR_ANIMACAO 0
#define ADDR_UNIDADE  1
#define ADDR_PONTEIRO 2
#define ADDR_LOG_INICIO 3

#define LOG_REGISTROS 50
#define LOG_BYTES     10
#define LOG_OPTION    1
#define EEPROM_CLEANER 1
...
int utc_offset = 0; // Fuso horário (Ex: Brasília UTC-3)
...
// Controle de tempo (millis) para não travar o loop
unsigned long anteriorMillis = 0;
unsigned long lastDebounceTime = 0;
unsigned long anteriorBuzzerMillis = 0;

const int intervaloBuzzer = 5000;
bool ledVermelhoAtivo = false;
bool alertaDisplay = false; // Guarda se o dado mostrado na tela atual está em alerta

const long intervalo = 10000; // Grava a cada 10 segundos
...
// Limpeza da EEPROM (Pode ser comentado após a primeira execução)
void limpaEEPROM()
{
    lcd.setCursor(0, 0);
    lcd.print("Limpando");
    lcd.setCursor(0, 1);
    lcd.print("EEPROM...");
    Serial.println("Limpando EEPROM... Aguarde.");
...
void loop() {
  ...
  // Verifica todos os sensores a cada 2 segundos (evita travar o Arduino com leituras do DHT)
  static unsigned long anteriorSensorMillis = 0;
  if (atualMillis - anteriorSensorMillis >= 2000) {
    anteriorSensorMillis = atualMillis;
    
    float t = dht.readTemperature();
    float h = dht.readHumidity();
    int l = map(analogRead(LDRPIN), 0, 1023, 0, 100);
    
    // Verifica se ALGUM deles está fora do limite (TRIGGER)
    bool alertaTemp = (t <= 15 || t >= 25);
    bool alertaUmid = (h <= 30 || h >= 70);
    bool alertaLuz  = (l < 0 || l >= 30);

    if (alertaTemp || alertaUmid || alertaLuz) {
      digitalWrite(PIN_LED_R, HIGH); // Liga o LED
      ledVermelhoAtivo = true;       // Libera o Buzzer
    } else {
      digitalWrite(PIN_LED_R, LOW);  // Desliga o LED
      ledVermelhoAtivo = false;      // Trava o Buzzer
    }
  }
  ...
  // Lógica do Buzzer: Apita a cada 5s se o alerta estiver ativo
  if (ledVermelhoAtivo) {
    if (atualMillis - anteriorBuzzerMillis >= intervaloBuzzer) {
      anteriorBuzzerMillis = atualMillis;
      tone(PIN_BUZZER, 1000, 1000);
    }
  } else {
    anteriorBuzzerMillis = atualMillis; // Trava o timer se o alerta acabar
  }

  // Verifica os sensores e grava no log periodicamente (independente do display)
...
void telaDados() {
  ...
  if (subTela == 0) {
    ...
    int porcentagemLDR = map(analogRead(LDRPIN), 0, 1023, 0, 100);
    lcd.print("*L: "); 
    lcd.print(porcentagemLDR); 
    lcd.print("%   "); 
  }

  if (digitalRead(botao_ok) == HIGH && (millis() - lastDebounceTime > debounceDelay)) {
    subTela = (subTela + 1) % 3;
    lcd.clear();
...
void verificaEGrava(DateTime horarioAtual) {
  ...
  int novaLuz = map(analogRead(LDRPIN), 0, 1023, 0, 100);
  if (isnan(novaTemp) || isnan(novaUmid)) return;

  float diffTemp = abs(novaTemp - ultimaTemp);
  float diffUmid = abs(novaUmid - ultimaUmid);
  int diffLuz    = abs(novaLuz - ultimaLuz);
  
  // Grava na EEPROM apenas se houver mudança nos valores
  if (diffTemp >= 0.5 || diffUmid >= 5.0 || diffLuz > 10) {
...
// --- Funções de Animação com Custom Characters ---
...
void wine2() {
  ...
  lcd.setCursor(5, 1); lcd.write(byte(2));
  lcd.setCursor(6, 1); lcd.write(byte(3));
  lcd.setCursor(7, 0); lcd.write(byte(4));
  lcd.setCursor(6, 0); lcd.write(byte(5));
}
